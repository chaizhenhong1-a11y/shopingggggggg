...
- 未登录用户点击收藏时会先进入登录页面。
- 新增商家后台，SELLER 和 ADMIN 账号可查看销售概览、商品和订单。
- 商家商品支持新增、编辑、调整库存及上下架。
- 商家订单只显示属于当前店铺的商品，并支持确认发货。
- 商家统计和操作 API 均经过 JWT 鉴权及店铺所有权校验。
- 管理后台改为独立的 `admin-web` Web 工程，不嵌入 Flutter 买家 App。
- Web 后台采用桌面侧边栏、销售仪表盘、库存图表和订单数据表结构。
- Flutter、管理后台和 Node.js API 保持独立分层，便于分别开发与部署。

...
class AddressPage extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(selectMode ? '选择收货地址' : '地址管理', style: const TextStyle(fontWeight: FontWeight.w900)), backgroundColor: Colors.white),
      body: rows.isEmpty ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.location_off_outlined, size: 64, color: Colors.black26), SizedBox(height: 12), Text('还没有收货地址')])) : RefreshIndicator(
        ...
        child: ListView.separated(padding: const EdgeInsets.all(14), itemCount: rows.length, separatorBuilder: (_, __) => const SizedBox(height: 10), itemBuilder: (_, i) { final address = rows[i]; return InkWell(
          ...
          child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Expanded(child: Text('${address.receiverName}  ${address.phone}', style: const TextStyle(fontWeight: FontWeight.w900))), if (address.isDefault) Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: const Color(0xFFFFE5DE), borderRadius: BorderRadius.circular(6)), child: const Text('默认', style: TextStyle(color: AppColors.primary, fontSize: 11)))]),
            const SizedBox(height: 9), Text(address.fullAddress, style: const TextStyle(color: AppColors.muted, height: 1.4)),
            if (!selectMode) ...[const Divider(height: 24), Row(children: [if (!address.isDefault) TextButton(onPressed: () => ref.read(addressProvider.notifier).makeDefault(address.id), child: const Text('设为默认')), const Spacer(), IconButton(onPressed: () => _showForm(context, ref, address), icon: const Icon(Icons.edit_outlined)), IconButton(onPressed: () => _delete(context, ref, address), icon: const Icon(Icons.delete_outline, color: Colors.redAccent))])],
          ])),
        ); }),
      ),
      floatingActionButton: FloatingActionButton.extended(onPressed: () => _showForm(context, ref, null), backgroundColor: AppColors.primary, foregroundColor: Colors.white, icon: const Icon(Icons.add), label: const Text('新增地址')),
    );
  }

  Future<void> _delete(BuildContext context, WidgetRef ref, ShippingAddress address) async { final yes = await showDialog<bool>(context: context, builder: (_) => AlertDialog(title: const Text('删除地址'), content: Text('确定删除 ${address.receiverName} 的地址吗？'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('取消')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('删除'))])); if (yes == true) await ref.read(addressProvider.notifier).remove(address.id); }
  ...
  Future<void> _showForm(BuildContext context, WidgetRef ref, ShippingAddress? old) async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (sheetContext) => StatefulBuilder(builder: (_, setSheetState) => Padding(padding: EdgeInsets.fromLTRB(20, 20, 20, MediaQuery.viewInsetsOf(sheetContext).bottom + 20), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
      Text(old == null ? '新增收货地址' : '编辑收货地址', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 16),
      _field(name, '收货人'), const SizedBox(height: 10), _field(phone, '手机号', type: TextInputType.phone), const SizedBox(height: 10), _field(line, '详细地址'), const SizedBox(height: 10), Row(children: [Expanded(child: _field(city, '城市')), const SizedBox(width: 10), Expanded(child: _field(state, '州属'))]), const SizedBox(height: 10), _field(postal, '邮政编码', type: TextInputType.number), SwitchListTile(value: isDefault, onChanged: (v) => setSheetState(() => isDefault = v), title: const Text('设为默认地址'), contentPadding: EdgeInsets.zero),
      SizedBox(width: double.infinity, child: FilledButton(onPressed: () async { if ([name, phone, line, city, state, postal].any((c) => c.text.trim().isEmpty)) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('请填写完整地址'))); return; } await ref.read(addressProvider.notifier).save(id: old?.id, receiverName: name.text, phone: phone.text, addressLine: line.text, city: city.text, stateName: state.text, postalCode: postal.text, isDefault: isDefault); if (sheetContext.mounted) Navigator.pop(sheetContext); }, child: const Text('保存'))),
    ])))));
